...
- [ ] 1.1 Refactor `packages/cli/src/lib/models.ts` into the per-vendor
      strategy table; export `SUPPORTED_VENDORS`, `isModelVendor`, and pure
      per-vendor parsers
- [ ] 1.2 OpenCode strategy: probe `["models"]`, line parser (CRLF-safe,
      `^\S+/\S+$` filter, provider from slash prefix, zero matches → failure)
- [ ] 1.3 Claude strategy: `native: { unavailableReason }` (curated string);
      bundled list becomes aliases `opus` / `sonnet` / `haiku`
- [ ] 1.4 Make `listModelsForVendor` / `detectActiveVendor` async via
      `execBinaryAsync`; capture probe stderr into
      `nativeUnavailableReason`; add 60s per-vendor TTL cache
- [ ] 1.5 Rewrite `models.test.ts` hermetically (`vi.mock` of
...
- [ ] 2.1 `packages/cli/src/commands/models.ts`: validate via
      `isModelVendor`; await async lib; print reason note; `--json` emits the
      envelope (and null envelope when no vendor detected)
- [ ] 2.2 `packages/dashboard/src/server/routes/team.ts`: async handler;
      vendor validation derives from `SUPPORTED_VENDORS`; passes
      `nativeUnavailableReason` through
- [ ] 2.3 Delete `listModels()` from `AiCliAdapter` interface, both
      adapters (and their bundled lists), and the session-capture test stub;
      delete the server-side duplicate `ModelDescriptor`
- [ ] 2.4 Contract test: every `AiCliService` registered adapter binary is in
      `SUPPORTED_VENDORS`
- [ ] 2.5 Amend `evolve-phase4-host-aware-spawning/tasks.md` 3.1–3.2 to
      register strategy-table entries instead of implementing `listModels`

## 3. Dashboard client

- [ ] 3.1 `api-types.ts`: add `nativeUnavailableReason?` to
      `ModelListResponse`
- [ ] 3.2 `ModelSelect`: render unknown non-empty values as a "(custom)"
      option; add always-available "Custom…" free-text escape (sentinel must
      not leak into team state)
- [ ] 3.3 Consolidate the team-composition-panel's inline `ModelPicker` onto
      the shared `ModelSelect`
- [ ] 3.4 Bundled-source hint (shared element) in panel, reviewer dialog,
...
- [ ] 4.3 `server-harness.ts`: opt-in `env` param; dashboard-api-e2e tests
      for `GET /api/team/models` (native success, malformed → bundled +
      reason, `vendor=auto` pin, unknown vendor 400)
- [ ] 4.4 Dispatch `cross-platform-test.yml` for `windows-latest` +
      `macos-latest` on the branch before merge (PR CI is ubuntu-only)

## 5. Verification

- [ ] 5.1 `pnpm nx run-many -t typecheck build test -p platform cli dashboard`
- [ ] 5.2 `pnpm nx e2e cli-e2e` and `pnpm nx e2e dashboard-api-e2e`
- [ ] 5.3 Manual smoke: `ocr models list` (both vendors) + dashboard dropdown
